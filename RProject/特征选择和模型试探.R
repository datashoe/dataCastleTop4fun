...
sampleData1 <- read.table(
  ...
  sep = ",",
  stringsAsFactors = F,
  fileEncoding = "UTF-8") # 数据分片1
colNames <- c("user_id","register_time","wood_add_value","wood_reduce_value",
              ...
              "bd_barrack_level","bd_healing_spring_level","bd_dolmen_level",
              "bd_guest_cavern_level","bd_warehouse_level","bd_watchtower_level",
              "bd_magic_coin_tree_level","bd_hall_of_war_level","bd_market_level",
              "bd_hero_gacha_level","bd_hero_strengthen_level","bd_hero_pve_level",
              "sr_scout_level","sr_training_speed_level","sr_infantry_tier_2_level",
              "sr_cavalry_tier_2_level","sr_shaman_tier_2_level","sr_infantry_atk_level",
              "sr_cavalry_atk_level","sr_shaman_atk_level","sr_infantry_tier_3_level",
              "sr_cavalry_tier_3_level","sr_shaman_tier_3_level","sr_troop_defense_level",
              ...
              "sr_troop_attack_level","sr_construction_speed_level","sr_hide_storage_level",
              ...
              "prediction_pay_price", "bucket", "dayOfWeek")

colnames(sampleData1) <- colNames # 列名

testDa <- sampleData1[1:1000, ] # 弄个小的数据用于debug

library(plyr)
testDa <- mutate(testDa, is_pay = prediction_pay_price > 0.99)

testDa[, "is_pay"] <- as.factor(testDa[, "is_pay"])
